 ...
 * Licence: MIT
 * Version: v2.0.1 (vendored, with SonoScript-specific patches below)
 *
 * SonoScript patches, both tagged "SonoScript patch" at their exact location:
 *   1. `options.target` in the public liquidGL() entry point now also accepts
 *      an actual Element (or array/NodeList of Elements), not only a CSS
 *      selector string — needed because the toolbar's target lives inside a
 *      CLOSED shadow root, which document.querySelectorAll cannot reach.
 *   2. Added liquidGLLens#destroy() and liquidGLRenderer#removeLens() — no
 *      equivalent existed upstream, since this library was built for glass
 *      elements that live for a whole page's lifetime, not a toolbar that
 *      gets opened, closed, and reopened repeatedly on the same page.
 */
...
(() => {
  ...
  class liquidGLRenderer {
    ...
    _initGL() {
      ...
      const fsSource = `
        ...
        // SonoScript patch: a SEPARATE blur from u_frost above, sampled at the UNDISTORTED base
        // position and reused for every texture lookup this shader makes (base color, the
        // refracted/aberrated sample, both aberration channels) — see sampleOurBlur() below and
        // its call sites. u_frost blurs AFTER the refraction offset has already been computed
        // (it scatters around sampleUV, the already-displaced position); this blurs the SOURCE
        // the displacement math itself reads from, so the geometric bend stays exactly as sharp
        // as bevelWidth/refraction say it should be, but the CONTENT being bent is soft — the
        // visual difference confirmed directly: blurring after refraction also softens the
        // aberration fringing and specular highlight edges along with the scene; blurring before
        // leaves those crisp and only the underlying image soft.
        uniform float u_ourFrost;
        ...
        vec2 cornerNormal( vec2 p, vec2 b, float r ) {
          // SonoScript patch: the upstream fallback direction here was normalize(p) — the
          // direction from the ELEMENT'S CENTER to the current point, passed in from main().
          // That's a purely radial vector field, which only looks right on roughly square
          // elements, where "away from center" and "perpendicular to the nearest edge" are
          // close to the same thing along most of an edge. This toolbar is a squat wide pill
          // (~12x wider than tall); for a point on the flat middle of the TOP edge but well off
          // to one side, "away from center" points mostly sideways, not straight up, so the
          // refraction field visibly converged toward the bar's centroid — a circular "lens
          // bulge" — instead of tracking the actual rounded-rect outline. It also made the
          // corner blend below jump between two very different-looking direction fields right at
          // the transition, which is what produced the jagged/glitchy rainbow seam right at the
          // corner. Confirmed both by tracing the math and by a live A/B screenshot comparison.
          // Fix: derive the fallback here, from the same p/b (real pixel-space, bar-relative
          // coordinates already passed in) rather than accepting an externally computed one — a
          // pure axis-aligned unit vector pointing straight out through whichever flat edge
          // (vertical or horizontal) is actually nearest. Everywhere along a flat edge this is
          // now correct regardless of position along that edge, and the corner blend now
          // interpolates between two direction fields that already agree closely near the
          // boundary, instead of two that disagree sharply.
          vec2 distToEdge = b - abs(p);
          vec2 s = vec2(p.x < 0.0 ? -1.0 : 1.0, p.y < 0.0 ? -1.0 : 1.0);
          vec2 straightDir = (distToEdge.x < distToEdge.y) ? vec2(s.x, 0.0) : vec2(0.0, s.y);

          vec2 q = abs(p) - b + r;
          vec2 m = max(q, 0.0);
          float l = length(m);
          if (l <= 0.0) return straightDir;
          float w = smoothstep(0.0, max(r * 0.5, 1.0), min(m.x, m.y));
          if (w <= 0.0) return straightDir;
          return normalize(mix(straightDir, s * (m / l), w));
        }
        ...
        // SonoScript patch: same random-disc scatter shape as liquidGL's own u_frost, offset by
        // different constants in the random() seed so the two don't produce an identical dither
        // pattern if a page somehow had both active. radiusTexels <= 0.0 short-circuits to a
        // plain sample so every call site can route through this unconditionally with zero cost
        // when the SonoScript overlay's blur is left at its default of 0.
        vec4 sampleOurBlur(vec2 uv, float radiusTexels) {
          if (radiusTexels <= 0.0) return texture2D(u_tex, uv);
          vec2 texel = 1.0 / u_textureResolution;
          vec4 sum = vec4(0.0);
          const int SAMPLES = 12;
          for (int i = 0; i < SAMPLES; i++) {
            float angle = random(uv + float(i) + 17.0) * 6.283185;
            float dist = sqrt(random(uv - float(i) - 17.0)) * radiusTexels;
            vec2 off = vec2(cos(angle), sin(angle)) * texel * dist;
            sum += texture2D(u_tex, uv + off);
          }
        ...
        void main(){
          ...
          vec2 refractDir = cornerNormal(p_px, b_px, u_radius);
          ...
          // SonoScript patch: every texture2D(u_tex, ...) below became sampleOurBlur(..., ourRadius)
          // — baseCol, both branches of liquidGL's own refrCol sampling, and both aberration
          // channels. That's what actually puts our blur BEFORE refraction/aberration in the
          // chain: the displacement math (offset, chroma) is unchanged and stays exactly as
          // precise as before, but every SOURCE pixel it reads is pre-softened, so the bend
          // itself stays sharp while the content being bent is soft — as opposed to sharpening
          // everything first and blurring the finished composite (which also softens the bend's
          // own edges and the specular highlight, not just the underlying scene).
          float ourRadius = u_ourFrost * 4.0;
          vec4 baseCol   = sampleOurBlur(mapped, ourRadius);
          ...
          if (u_frost > 0.0) {
              ...
              for (int i = 0; i < SAMPLES; i++) {
                  ...
                  sum += sampleOurBlur(sampleUV + offset, ourRadius);
              }
              refrCol = sum / float(SAMPLES);
          } else {
              refrCol = sampleOurBlur(sampleUV, ourRadius);
              refrCol += sampleOurBlur(sampleUV + vec2( texel.x, 0.0), ourRadius);
              refrCol += sampleOurBlur(sampleUV + vec2(-texel.x, 0.0), ourRadius);
              refrCol += sampleOurBlur(sampleUV + vec2(0.0,  texel.y), ourRadius);
              refrCol += sampleOurBlur(sampleUV + vec2(0.0, -texel.y), ourRadius);
              refrCol /= 5.0;
          }
          ...
          if (u_aberration > 0.0) {
              ...
              refrCol.r = sampleOurBlur(sampleUV - chroma, ourRadius).r;
              refrCol.b = sampleOurBlur(sampleUV + chroma, ourRadius).b;
          }
      ...
      this.u = {
        ...
        ourFrost: gl.getUniformLocation(this.program, "u_ourFrost"),
    ...
    // SonoScript patch: pairs with liquidGLLens#destroy() above — no
    // equivalent removal existed upstream (this library was written for
    // glass elements that live for the whole page's lifetime, not a
    // toolbar that's opened and closed repeatedly on the same page).
    removeLens(lens) {
    ...
    _renderLens(lens) {
      ...
      // SonoScript's own blur, read live from the same lens.options bag as every official
      // liquidGL field above — `ourFrost` was never in liquidGL's own defaults, but the
      // constructor assigns lens.options = the raw options object we passed at init with no
      // whitelist/copy, confirmed directly, so an unofficial extra key here survives and stays
      // live-mutable exactly like refraction/aberration/etc. already are.
      gl.uniform1f(this.u.ourFrost, lens.options.ourFrost || 0);
  ...
  class liquidGLLens {
    ...
    // SonoScript patch: no equivalent existed upstream — needed because our
    // toolbar can be opened/closed/reopened repeatedly within the same page
    // load (unlike a typical page's one-shot glass card), so leaving stale
    // lenses in renderer.lenses forever, plus their own per-lens
    // ResizeObserver, is a real leak over a long browsing session.
    destroy() {
      if (this._sizeObs) {
  ...
  window.liquidGL = function (userOptions = {}) {
    ...
    // SonoScript patch: options.target may be an actual Element / array of
    // Elements / NodeList, not just a CSS selector string — needed because
    // our target lives inside a CLOSED shadow root, which document-level
    // querySelectorAll structurally cannot reach (that's the whole point of
    // "closed" mode). Resolves to an array of elements either way; falls
    // through to the original document.querySelectorAll(string) behavior
    // unchanged for normal (non-shadow-root) usage.
    function sonoscriptResolveTargets(target) {
    ...
    if (noWebGL) {
      ...
      const fallbackNodes = sonoscriptResolveTargets(options.target);
    ...
    const nodeList = sonoscriptResolveTargets(options.target);
