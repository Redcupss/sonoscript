...
LOCAL_TTS_PACKAGES = [
    ...
    # all do importlib.metadata lookups at import/runtime the same way phonemizer/kokoro_onnx
    # do, so they need the same dist-info-alongside-package treatment. NOTE: "mlx" itself is
    # deliberately NOT here — it's a namespace package (no __init__.py, standard for packages
    # built around a compiled core), and py2app's "packages" option crashes on that shape
    # (ImportError inside py2app's own get_bootstrap(), which uses a legacy imp.find_module()-
    # style lookup that predates PEP 420 namespace packages and can't handle them at all). mlx
    # is instead copied verbatim in the post-setup() step below, the same "take explicit
    # control of a whole directory tree" approach already used for kokoro_assets. This list is
    # a starting point based on mlx-audio's declared deps plus the obvious transitive ML stack
    # — expect to discover more missing entries via real ModuleNotFoundError/ImportError when
    # the frozen app actually launches, same iterative process that shaped every other block.
...
OPTIONS = {
    ...
    "excludes": ["mlx"],
    "resources": ["kokoro_assets", "chatterbox_assets"],
    "iconfile": "icon.icns",
}
...
# control approach as kokoro_assets/chatterbox_assets, so nothing about its non-code assets
# (mlx.metallib, the Metal compute shader binary; libmlx.dylib/libjaccl.dylib, its C++ core
# libraries) depends on modulegraph's default tracing correctly guessing what to include.
import shutil
